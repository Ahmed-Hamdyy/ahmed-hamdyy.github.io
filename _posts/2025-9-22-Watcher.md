...
description: This is a walkthrough for Watcher room We will go through getting ftp credentials from a file on the website by abusing an lfi vulnerability, Gainning Access from the ftp through a php reverse shell, do some lateral movement between users untill we get access to a user with specific priviliges to see a hidden ssh key for the root authenticate with it and gain root access.
---
